...
| 变体 | 总收益 | Sharpe | Calmar | MaxDD | 暴露 |
|------|-------:|-------:|-------:|------:|-----:|
| 买入持有 (100%) | `+1022.58%` | `0.908` | `0.569` | `-76.63%` | `100%` |
| 纯底仓 (15%) | `+44.40%` | `1.033` | `0.805` | `-7.02%` | `41.3%` |
| 纯信号 (100%) | `+48.42%` | `0.760` | `0.516` | `-11.80%` | `6.8%` |
| 组合（当前） | `+100.23%` | `1.028` | `0.855` | `-12.81%` | `42.2%` |

信号层 Calmar 增量 = `0.855 - 0.805 = +0.05`。解读：

1. 纯底仓（15% 暴露的趋势跟踪）单独运行就能达到 Sharpe 1.033，与组合的
   1.028 几乎相同。
2. 复杂周期信号在风险调整口径上的增量贡献极小。
3. 解冻后的正确方向可能是**简化**（保留底仓 + 少量高置信信号）而非升级。
