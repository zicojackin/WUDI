...
### 解冻前归因快照（2026-09-04，历史数据，in-sample）

归因口径：**增量归因 + 风险调整**（信号层价值 = 组合 Calmar - 纯底仓 Calmar）。
同区间（2020-01-01 至 2026-09-04）、同成本假设。以下是描述性数据，
不作为解冻后的绩效基准。

实验条件：四变体均从空仓启动，同一数据文件，同一成本假设（手续费 0.1% +
滑点 0.05% + 资金费率 10% 年化）。组合的日收益构造为
`0.85 × signal_daily + base_daily`（验证残差 < 1e-15）。

| 变体 | 总收益 | Sharpe | Calmar | MaxDD | 暴露 | 收益/暴露 |
|------|-------:|-------:|-------:|------:|-----:|---------:|
| 买入持有 (100%) | `+1022.58%` | `0.908` | `0.569` | `-76.63%` | `100%` | `10.23` |
| 纯底仓 (15%) | `+44.40%` | `1.033` | `0.805` | `-7.02%` | `41.3%` | `1.07` |
| 纯信号 (100%) | `+48.42%` | `0.760` | `0.516` | `-11.80%` | `6.5%` | **`7.16`** |
| 组合（当前） | `+100.23%` | `1.028` | `0.855` | `-12.81%` | `42.2%` | `2.38` |

**算术校验**：组合累计 `100.23%` ≠ 缩放信号累计 `40.28%` + 底仓累计 `44.40%`
= `84.68%`，差值 `+15.56%` 为复利交叉项（`prod(1 + 0.85s + b)` 与
`prod(1 + 0.85s) + prod(1 + b) - 1` 的差异），不是来源不明的收益。

信号层 Calmar 增量 = `0.855 - 0.805 = +0.050`。关键发现：

1. 纯底仓 Sharpe (1.033) 与组合 (1.028) 几乎相同：复杂信号在风险调整口径上
   的增量很薄。
2. **信号层的资金效率是底仓的 6.7 倍**（7.16 vs 1.07），但它只用了 6.5% 的
   市场暴露时间。信号层时间加权收益率是底仓的约 11 倍（7.45 vs 1.07）。
   信号层不是没用，而是资本利用率极低。
3. 解冻后的核心问题不是"简化还是升级"，而是：**能否在不显著增加回撤的前提下
   提高信号层的暴露率**。暴露率 6.5% 在趋势策略里是极端值，可能原因按优先级：
   入场条件太严 / 出场太早 / 4H 确认过滤过多信号。解冻后第一周逐层排查。
