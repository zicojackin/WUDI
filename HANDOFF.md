...
|---|---:|---:|---:|---:|---:|---|
| BTC 组合 | `+100.23%` | `1.028` | `-12.81%` | `42.15%` | `~0.97` | 42 笔信号层交易，信号 IR `0.760` |
...
- 零 repaint 事件（信号不因盘中价格变化而反复翻转）
- `state.json` 按收盘 K 线推进，无跳跃或遗漏
- 填充价与 T+1 开盘价偏差 < `0.5%`（长期均值）
- 模拟盘状态转换与手工核对一致
...
1. 样本量少，BTC / ETH 各只有 7-8 笔交易，统计意义不足。
2. ETH 已平仓交易质量弱，胜率低，止损和反转信号有效性需要逐笔复盘。
3. 出场主要依赖 ATR，结构化出场、均线出场和时间止损尚未实现。
4. 资金费率目前是固定年化假设，未使用 OKX 历史 funding rate。
5. Walk-forward 还没有在最新的 `daily` 模式下完整重跑。
6. 回测指标不足，目前缺少 Sharpe / Sortino、年化收益、连续亏损等风险调整指标。
7. 数据完整性校验还没有独立脚本。
8. 文档和代码仍未提交，无法用 commit hash 精确复现。
9. P4-P6 模块尚未接入主回测，当前只作为独立组件。
10. Walk-forward 显示 ETH 参数稳定性明显不足，BTC 也仍有较多非正收益窗口。
11. 2026-09-04 诊断显示，BTC 持仓期间基准收益远高于策略收益，说明出场过早。
12. ETH 亏损集中在 `accumulation`，Reversal Extension 平均亏损。
13. 参数扰动显示 BTC 对 `min_setup_score` 和 `min_pattern_quality` 中等敏感；
    ETH 对 `trail_atr_multiple` 中等敏感。
14. BTC Walk-forward 一致性在 v0.8 仍只有 `9/28`，说明长期正收益依赖少数窗口。
15. `ETHStrategyV3` 是独立策略层，尚未替换主 `cycle.py` 入场引擎。
